...
It’s not that sophisticated, rather it’s a simplistic application that needs to interact with a database. 
It’s a classic PHP application, if you will.
...
. A PHP runtime (I’ll be using PHP {php-runtime-version})
. A webserver to serve the static content.
  I've chosen NGINX over Apache.
...
// TODO: Cover shared volumes, environment variables, healthcheck, labels, secrets, restart
